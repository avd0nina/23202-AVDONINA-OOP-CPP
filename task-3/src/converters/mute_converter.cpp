...
void RawMuteConverter::convert(std::string_view command,
    ...
    // Убираем изменение громкости, оставляем только заглушение
    ...
    for (size_t i = start_sample; i < end_sample; i++){
        // Вместо изменения громкости, просто пишем нули (заглушение)
        WAVFormatInfo::Sample silent_sample = 0;
        output_stream.write(silent_sample);
    }

    // Оставшиеся данные из входного потока записываем без изменений
