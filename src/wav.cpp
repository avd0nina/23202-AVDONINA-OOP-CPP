...
struct FMTSubchunk{ //описываем формат данных
    uint32_t subchunk_size; // Размер оставшейся части подзаголовка (обычно 16 для PCM)
    uint16_t audio_format; // Формат аудио: 1 для PCM
    uint16_t num_channels; // Количество каналов: поддерживается только 1 (моно)
...
void read_extra_data_subchunk(std::ifstream& wav_file){ //считываем метаданные в буфер
...
struct WAVManagement::WAVReader::Impl{
    WAVFormatInfo::Sample buffer[WAV_READER_BUFFER_SIZE]; // массив, в который загружаются сэмплы аудиоданных из WAV-файла.
