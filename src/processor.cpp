...
struct Processing::Processor::Impl{ // скрываям внутренности
    std::string_view output_file_path;  // название выходного файла
    std::string_view input_file_path; // название входного файла
    std::vector<Streams::InputStream> additional_streams;// названия дополнительных входных файлов
    ConfigParsing::ConfigParser config_parser; // парсер конфига
    const Factory::ConvertersFactory& factory; // фабрика конвертеров
    Converters::ConverterParams create_params(const std::vector<std::string>& command_args); // создание параметров конвертера
};

Converters::ConverterParams Processing::Processor::Impl::create_params(const std::vector<std::string>& command_args){ // создание параметров конвертера
...
void Processing::Processor::process() const{
    ...
    if (std::filesystem::exists(_pimpl->output_file_path)){ //если выходной файл уже существует
