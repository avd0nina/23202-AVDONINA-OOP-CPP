...
int checkNum(const std::string &num) { 
    ...
    for (size_t i = 0; i < num.size(); ++i) {
        if (!isdigit(num[i]) && num[i] != '-') {
            throw std::invalid_argument("Tick is incorrect");
        }
    }
    //str->int
    return stoi(num);
...
SimulationController::SimulationController(
    ...
    try {
        if (infile.size() < 4 || infile.substr(infile.size() - 5, 5) == ".txt" || !ifile.is_open()) { 
            throw std::invalid_argument("You inserted the wrong input file");
        }
        tick = checkNum(ticks);
    ...
    try {
        if (!out_file.empty()) {
            ...
            if (out_file.size() < 4 ||
                ...
                !ofile.is_open())
            {
                throw std::invalid_argument("Wrong output file");
            }
        ...
        while (command != "exit") {
            if (command.substr(0, 4) == "tick") {
                ...
                if (command.size() < 9 || command.substr(command.size() - 5, 5) == ".txt") {
                    view_.printErr("Wrong output file");
                } else {
